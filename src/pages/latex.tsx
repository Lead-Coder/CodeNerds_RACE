...
export default function LaTeXEditor() {
  ...
  // Load LaTeX file from public/output/tex.tex on mount
  useEffect(() => {
    fetch('/output/tex.tex')
      .then((res) => {
        if (!res.ok) throw new Error('Failed to load tex file');
        return res.text();
      })
      .then((data) => setLatexCode(data))
      .catch((err) => {
        console.error(err);
        setLatexCode('% Failed to load LaTeX content');
      });
  }, []);
